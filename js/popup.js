...
const LANGUAGES = {
  ...
  en: {
    ...
    modal: {
      ...
      resetWarning: 'This will clear current authorization status, you need to re-authorize when visiting Luma page.',
      cancel: 'Cancel',
      confirmReset: 'Confirm Reset',
      cookieTitle: 'Cookie Usage Authorization',
      cookieMessage: 'EventMate needs to read authentication cookies from current page to access event data you have permission to view.',
    ...
    try {
      ...
      if (response && response.loaded) {
        ...
        if (response.eventsListVisible && response.eventsCount > 0) {
          this.updatePluginStatusUI(`${LanguageManager.getText('messages.eventListLoaded')} (${response.eventsCount}${LanguageManager.getText('messages.events')})`, true);
    ...
    if (cookieModal) {
      ...
      if (promiseList) {
        ...
        if (Array.isArray(promises)) {
          promiseList.innerHTML = promises.map((promise, index) => 
            `<li data-i18n="modal.promiseList.${index}">${promise}</li>`
          ).join('');
        }
      }
    }
